...
const buildOffboardingRows = async () => {
  ...
  return records.map((record) => ({
    "Employee ID": record.employeeCode || "",
    Employee: record.employeeName || record.name || "",
    Department: record.department || "",
    Manager: record.managerName || "",
    "Joining Date": formatDate(record.joiningDate),
    "Exit Type": capitalize(record.exitType || record.reason || ""),
    "Notice Period": record.noticePeriod || "",
    "Last Working Day": formatDate(record.lastWorkingDay || record.lastDay),
    "Actual Last Working Day": formatDate(record.actualLastWorkingDay),
    "Exit Interview Status": capitalize(record.exitInterviewStatus || "pending"),
    "HR Clearance": capitalize(record.clearanceStatus?.hr || "pending"),
    "IT Clearance": capitalize(record.clearanceStatus?.it || "pending"),
    "Finance Clearance": capitalize(record.clearanceStatus?.finance || "pending"),
    "Assets Return Status": capitalize(record.assetsReturnStatus || "pending"),
    "FNF Status": capitalize(record.fnfStatus || "pending"),
    "Rehire Eligibility": capitalize(String(record.rehireEligibility || "under_review").replaceAll("_", " ")),
